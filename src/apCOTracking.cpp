...
//       truthpath = env_ipath2 + vpIoTools::path("/truth.txt");
    ...
    opath = vpIoTools::path("outPig7") + vpIoTools::path("/image%06d.png");
    opath1 = vpIoTools::path("outPig7") + vpIoTools::path("/imageKalman%06d.png");
...
//    vpMbPointsTracker tracker2;
   ...
   // vpOpenCVGrabber grabber;
    ...
    std::string filenameLiver = "/home/antoine/Documents/imagesLiverPig5/image000000.png";
    std::string filenameLiverCol = "/home/antoine/Documents/imagesLiverPig5/imageColor000000.png";
    vpImageIo::read(Id, filenameLiver) ;
    ...
    vpImageIo::read(Icol, filenameLiverCol) ;
    }
    const int width = Icol.getWidth();
    const int height = Icol.getHeight();
...
std::cout << " witdth " << mcam.get_px() << " height " << mcam.get_py() << std::endl;
        ...
        while(true){
            ...
            vpImageIo::write(Ioverlaycol, filename4);
                    	...
                    	for (int i = 0; i<height; i++)
                    		for (int j = 0; j<width; j++)
                    		{
                                        if(Inormd[i][j].R == 127 && Inormd[i][j].G == 127 && Inormd[i][j].B == 127)
					{
                    			Inormd[i][j].R = 0;
                    			Inormd[i][j].G = 0;
                    			Inormd[i][j].B = 0;
