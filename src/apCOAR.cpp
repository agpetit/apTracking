...
void createScene()
  {
    // Lumieres
    mSceneMgr->setAmbientLight(Ogre::ColourValue((float)1,(float)1,(float)1)); // Default value of lightning
    ...
    Ogre::Light * light0 = mSceneMgr->createLight();
    light0->setDiffuseColour(1.0,1.0,1.0); // scaled RGB values
    light0->setSpecularColour(1.0,1.0,1.0); // scaled RGB values
    // Lumiere ponctuelle
    ...
    // Create the Entity
    robot = mSceneMgr->createEntity("Yoshi", "yoshi2.mesh");
    //robot->setMaterialName("/local/agpetit/soft/apTracking/mbt-co/build/yoshi");
    // Attach robot to scene graph
    Ogre::SceneNode* RobotNode = mSceneMgr->getRootSceneNode()->createChildSceneNode("Yoshi");
    RobotNode->attachObject(robot);
    RobotNode->scale((Ogre::Real)0.1,(Ogre::Real)0.1,(Ogre::Real)0.1);
...
//       truthpath = env_ipath2 + vpIoTools::path("/truth.txt");
     ...
     if (configFile.empty())
      ...
      modelFileAR = vpIoTools::path("yoshi/yoshi.obj");
...
grabber.acquire(Idisplay);*/
    ...
    // Create a vpRAOgre object with color background
      vpAROgreExample ogre0(mcam, width, width);
      // Initialize it
      ogre0.init(IC);

      vpHomogeneousMatrix mMy;
      //vpTranslation;
    ...
    try
    {
        while(true){
            ...
            try{
                ...
                //mgrAR->updateRTTCol(Inormd0,Inormd,Ior,&cMo);
                ...
                posev[5] = posear[im][5];
                cMo.buildFrom(posev);
                std::cout<<" cMo in" << cMo <<std::endl;
                IC = Icol;
                ogre0.display(IC,cMo);*/
                //vpDisplay::getClick(Id);
                //vpImageIo::writePNG(Inormd, "Inormd.png");
                //vpImageIo::writePNG(Inormd0, "Iar.png");
            }
            catch(...){
           ...
           /*for (int i=0;i<height-0;i++)
            ...
            //vpImageIo::write(Inormd, filename4);
            im++;
        }
