...
// 		vpDisplay::display(I);
		...
		for(unsigned int i(0);i<nbclasses;++i){
			if (class_size [i] > (int)(rate_nbpts_min*nbGoodSites)){
			    ...
			    /*if(i==2)
			    vpFeatureDisplay::displayLine(mean_lines[i]->getRho(),mean_lines[i]->getTheta(), *cam, I, vpColor::green, 1) ;
			    if(i==1)
...
// 				cout << "err["<< i <<"] = " <<fabs(err[i]) << endl;
		...
		// 				double wght = err.euclideanNorm()/(float)err.getHeight();
				...
				//double wght = weights[i]/wghtml;

				std::cout << " weight line " << wght << std::endl;
