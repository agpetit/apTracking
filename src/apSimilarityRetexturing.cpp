...
int main(int argc, char **argv)
{
    ...
    std::string vpath;
    std::string opt_ipath, opt_ipath1;
    std::string ipath, ipath1;
    std::string isegpath;
    std::string scpath;
    std::string configFile;
...
//    std::string truthpath;
    ...
    if (!getOptions(argc, argv, opt_object, opt_ipath, opt_ipath1, opt_display, start_image, configFile, opt_learn, opt_detect, inline_init)) {
      return (-1);
    }
...
//       truthpath = env_ipath2 + vpIoTools::path("/truth.txt");
     ...
     if (configFile.empty())
      ...
      std::string plotrt = object + vpIoTools::path("plotfilert.dat");

      char *plotfile0 = (char *)plot0.c_str();
      char *plotfile1 = (char *)plot1.c_str();
      char *plotfilert = (char *)plotrt.c_str();
...
//    vpMbPointsTracker tracker2;
    ...
    reader.open(Id);
    reader.acquire(Id);
    vpVideoReader readerRGB, readerRGB1;
    {
        ...
        readerRGB1.setFileName(ipath1.c_str());
        readerRGB1.setFirstFrameIndex(start_image-6);
        readerRGB1.open(Icolrt);
        readerRGB1.acquire(Icolrt);
    }
...
//    	  (*imArg)[y][x].A = (double)(*I00)[y][x];
    ...
    //Create a window (700 by 700) at position (100, 200) with two graphics
      ...
      vpPlot B(1, 700, 700, 100, 200, "Curves...");
      //The first graphic contains 1 curve and the second graphic contains 2 curves
      B.initGraph(0,2);
      //The color of the curve in the first graphic is red
      B.setColor(0,0,vpColor::blue);
      B.setColor(0,1,vpColor::red);
    ...
    try
    {
        while(true){
           ...
           // vpDisplay::displayRectangle(Icol, rect.y-margin, rect.x-margin, rect.width + 2*margin, rect.height + 2*margin, vpColor::red, false,2);
            vpDisplay::flush(Icol);
            ...
            vpImageConvert::convert(Imask,imask);
            vpImageConvert::convert(Imask,imask0);
            cvFindContours(imask, storage, &contours );

            //cv::imwrite("imask.png",imask0);
                   ...
                   std::cout << " rect " << rect.width << " " << rect.height << " " << rect.x << " " << rect.y <<  std::endl;


            for (int k = 0; k < Icol.getWidth(); k++)
                for (int l = 0; l < Icol.getHeight(); l++){
                    errorRT+= sqrt( (Icol[l][k].R - Icolrt[l][k].R)*(Icol[l][k].R - Icolrt[l][k].R) + (Icol[l][k].G - Icolrt[l][k].G)*(Icol[l][k].G - Icolrt[l][k].G) + (Icol[l][k].B - Icolrt[l][k].B)*(Icol[l][k].B - Icolrt[l][k].B));
                    if (l > rect.y - margin && k > rect.x - margin && l < rect.y + rect.height + margin && k < rect.x + rect.width + margin)
                    {
                        errorRTBB+= sqrt( (Icol[l][k].R - Icolrt[l][k].R)*(Icol[l][k].R - Icolrt[l][k].R) + (Icol[l][k].G - Icolrt[l][k].G)*(Icol[l][k].G - Icolrt[l][k].G) + (Icol[l][k].B - Icolrt[l][k].B)*(Icol[l][k].B - Icolrt[l][k].B));
                    }

                    }

                               std::cout << " errorRT " << (double)errorRT << std::endl;

            //vpImage<vpRGBa>

            B.plot(0,0,im-start_image+1,(double)errorRT/(Icol.getWidth()*Icol.getHeight()));
            B.plot(0,1,im-start_image+1,(double)errorRTBB/((rect.height + 2*margin)*(rect.width + 2*margin)));
            ...
            //vpDisplay::getClick(Id);
            fout << im << ' ' << (cMo)[0][3] << ' ' << (cMo)[1][3] << ' ' << (cMo)[2][3] << ' ' << (cMoMes)[0][3] << ' ' << (cMoMes)[1][3] << ' ' << (cMoMes)[2][3] <<   std::endl;
            // Write images
           ...
           // vpImageIo::write(Ioverlaycol, filename4);
                vpImageIo::write(Icol2, filename4);
