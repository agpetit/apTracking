...
void apHOGDetector::get_svm_detector(const Ptr<SVM>& svm, vector< float > & hog_detector )
{
...
*/
void apHOGDetector::convert_to_ml(const std::vector< cv::Mat > & train_samples, cv::Mat& trainData )
{
    //--Convert data
...
void apHOGDetector::load_images( const string & prefix, const string & filename, vector< Mat > & img_lst )
{
    string line;
    ifstream file;
...
void apHOGDetector::load_imagesL( const string & prefix, const string & filename, vector< Mat > & img_lst, int i )
{
    string line;
    ifstream file;
...
void apHOGDetector::sample_neg( const vector< Mat > & full_neg_lst, vector< Mat > & neg_lst, const Size & size )
{
    Rect box;
...
Mat apHOGDetector::get_hogdescriptor_visu(const Mat& color_origImg, vector<float>& descriptorValues, const Size & size )
{
...
void apHOGDetector::compute_hog( const vector< Mat > & img_lst, vector< Mat > & gradient_lst, const Size & size )
{
    HOGDescriptor hog;
    hog.winSize = size;
    Mat gray;
...
void apHOGDetector::compute_one_hog( const Mat & img, Mat & gradient, const Size & size )
{
    HOGDescriptor hog;
    hog.winSize = size;
    Mat gray;
...
void apHOGDetector::train_svm( const vector< Mat > & gradient_lst, const vector< int > & labels )
{
...
void apHOGDetector::train_svmL( const vector< Mat > & gradient_lst, const vector< int > & labels, int i )
{
...
void apHOGDetector::draw_locations( Mat & img, const vector< Rect > & locations, const Scalar & color )
{
    if( !locations.empty() )
    {
...
void apHOGDetector::test_itL( cv::Mat &img, const Size & size, int i , vector< Rect > & locations, vector< double > &weights)
{
    char key = 27;
    ...
    {
        //std::cout << " ok test_itl 2 " << std::endl;
        ...
        my_hog.detectMultiScale( img, locations, weights, 0.4, Size(8,8), Size(16,16), 1.05, true);

        //std::cout << " ok test_itl 3 " << std::endl;
        ...
        if (weights[kk]>max_weight)
        {
            max_weight = weights[kk];
            weights[0] = weights[kk];
            locations[0] = locations[kk];
        }


        int pos_i = locations[kk].y + (locations[kk].height)/2;
        int pos_j = locations[kk].x + (locations[kk].width)/2;

        //if (scale > 1.02)
        std::cout << " pos_i " << pos_i << " pos_j " << pos_j << " weight " <<weights[kk] << std::endl;
...
void apHOGDetector::test_it( const Size & size )
{
    char key = 27;
    Scalar reference( 0, 255, 0 );
    Scalar trained( 0, 0, 255 );
...
vpHomogeneousMatrix apHOGDetector::getTemplatePose(double scale, int i, int j, vpCameraParameters &cam, vpHomogeneousMatrix &cMo)
{
...
void apHOGDetector::train(const string & pos_dir, const string & pos, const string & neg_dir, const string & neg)
{
...
void apHOGDetector::detect(vpImage<unsigned char> &_I, std::string &_path, vpHomogeneousMatrix &cMo_)
{
    ...
    vpImageConvert::convert(_I, img);

    cv::resize(img, imgS, Size( 200, (int)(((double)img.rows/img.cols)*200) ) );


    vpCameraParameters camparam;
    camparam.initPersProjWithoutDistortion(725,725,imgS.rows/2,imgS.cols/2);

    double max_weight = 0;
    int indice_maxweight = 0;
    ...
    std::cout << " ok detect " << i << std::endl;
        ...
        //if (scale > 1.02)
        std::cout << " scale " << scale << " pos_i " << pos_i << " pos_j " << pos_j << " " << camparam.get_u0() << " " << camparam.get_v0() << " size weights " << weights.size() << std::endl;

        cMoTrans = getTemplatePose(scale,pos_i, pos_j , camparam, cMo);

        cMo_ = cMoTrans;

      std::cout << "dvalue1 " << dvalue1 <<  " cMo " << cMo << " cMoTrans " << cMoTrans << std::endl;
