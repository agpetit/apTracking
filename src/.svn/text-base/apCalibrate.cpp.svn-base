...
//---------PARAMETERS--------------------
  ...
  const unsigned int nptPose = 12; //number of init dots by image
  vpPoint P[nptPose];
  //plan xOy
     ...
     P[0].setWorldCoordinates(6.40,17.94, 2.81);
     P[1].setWorldCoordinates(5.33,14.53,3.34);
     P[2].setWorldCoordinates(4.1, 11.08, 3.89 );
     P[3].setWorldCoordinates(-2.77,9.49, 5.94);
     P[4].setWorldCoordinates(3.26, 7.66, 4.46);
     P[5].setWorldCoordinates(3.26,-7.63,4.47);
     P[6].setWorldCoordinates(-2.77, -9.48 ,5.94);
     P[7].setWorldCoordinates(4.14,-11.10,3.89);
     P[8].setWorldCoordinates(5.3,-14.57,3.37 ) ;
     P[9].setWorldCoordinates(6.4,-17.94,2.8) ;
     P[10].setWorldCoordinates(10.07,-30,-1.56 ) ;
     P[11].setWorldCoordinates(10.07,29.97 ,-1.56) ;
...
//---------------------------------------------------
  ...
  if (opt_ppath.empty()){
    ...
    dirname = ipath + vpIoTools::path("/imagesRefuel/Refuel1/");
  ...
  while (iter < opt_first + opt_nimages*opt_step) {
    ...
    try{
      for(unsigned int i=0;i<nptPose;i++) {
        ...
        // The default of this setting is that it is time consumming
    	  ...
    	  vpDisplay::getClick(I,ip_click[i],true);
    	  vpDisplay::displayCross(I,ip_click[i], 5,vpColor::green);
          vpDisplay::flush(I);
    ...
    for (unsigned int i=0 ; i < nptPose ; i++){
      ...
      // where px, py, u0, v0 are the intrinsic camera parameters
    	//vpImagePoint ip0;
    	//vpDisplay::getClick(I,ip0,true);
      double x=0, y=0;
      //vpImagePoint cog = d[i].getCog();
      //vpImagePoint cog = ip_click[i].getCog();
      //vpPixelMeterConversion::convertPoint(cam, cog, x, y);
      vpPixelMeterConversion::convertPoint(cam, ip_click[i], x, y);
      P[i].set_x(x) ;
      P[i].set_y(y) ;
    }
    ...
    for (unsigned int i=0 ; i < nptPose ; i++){
      ...
      calib.addPoint(P[i].get_oX(),P[i].get_oY(),P[i].get_oZ(), ip_click[i]);

      table_cal[niter].addPoint(P[i].get_oX(),P[i].get_oY(),P[i].get_oZ(),  ip_click[i]) ;
