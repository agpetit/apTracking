...
void
vpFeatureGradient::buildOr1From(vpImage<unsigned char> &I, vpImage<unsigned char> &I2)
{
  int    l = 0;
  double Ix,Iy, Ixx, Ixy, Iyx, Iyy ;
  ...
  //  if (firstTimeIn==0)
      ...
      for (int i=3; i < nbr-3 ; i++)
	{
	  ...
	  for (int j = 3 ; j < nbc-3; j++)
	    {
	      // cout << dim_s <<" " <<l <<"  " <<i << "  " << j <<endl ;
	      Ix =   px*vpImageFilter::derivativeFilterX(imG,i,j) ;
	      Iy =   py*vpImageFilter::derivativeFilterY(imG,i,j) ;
	      imIx[i][j] = Ix ;
	      imIy[i][j] = Iy ;
	    }
	}
	  ...
	  for (int j = bord ; j < nbc-bord; j++)
	    {
	      ...
	      {
	      s[l]=vpMath::sqr(atan(Iy/Ix));
	      ip.set_i(i);
	      ip.set_j(j);
	      vpDisplay::displayCross(I2,i,j,1,vpColor::red,1);
	      //cout<<"ssss  "<<i<<"  "<<j<<endl;
	      m++;
	      }
	      else {
		  s[l]  = vpMath::sqr(M_PI/2) ;
	      }
	      //cout<<"ssss"<<s[l]<<endl;
	      //s[l]  = vpMath::sqr(Ix) + vpMath::sqr(Iy);
...
vpFeatureGradient::interactionOr1(vpMatrix &L)
{
	...
	double A,B,C,D,E,F,x,y,Ix,Iy,Ixx,Ixy, Iyx, Iyy, Zinv;
  ...
  for(int m = 0; m< L.getRows(); m++)
    {
      ...
      if (sqrt(vpMath::sqr(Ix)+vpMath::sqr(Iy))>2)
      {
             A = 2*atan(Iy/Ix)*(( Ixx*Iy-Ixy*Ix ))*(1/(vpMath::sqr(Ix)+vpMath::sqr(Iy)));
             B = 2*atan(Iy/Ix)*(( Iyx*Iy-Iyy*Ix ))*(1/(vpMath::sqr(Ix)+vpMath::sqr(Iy)));
             C = 2*atan(Iy/Ix)*(-Ix*Iy*(1/(vpMath::sqr(Ix)+vpMath::sqr(Iy))));
             D = 2*atan(Iy/Ix)*(-vpMath::sqr(Ix)*(1/(vpMath::sqr(Ix)+vpMath::sqr(Iy))));
             E = -C;
             F = 2*atan(Iy/Ix)*(vpMath::sqr(Iy)*(1/(vpMath::sqr(Ix)+vpMath::sqr(Iy))));
	...
	L[m][0] = -A * Zinv;
	L[m][1] = -B * Zinv;
	L[m][2] = (A*x+B*y)*Zinv+(C+E)*Zinv;
	L[m][3] = A*x*y + B*(1+y*y)+2*y*C+y*E+x*F;
	L[m][4] = -A*(1+x*x) - B*x*y-x*C-y*D-2*x*E;
	L[m][5]  = A*y - B*x - D + F;
      }
    }
}
			  ...
			  vpColVector &e)
{
  ...
  for (int i =0 ; i < dim_s ; i++)
    {
	  /*e0=abs(s[i] - s_star[i]-M_PI);//*(s[i] - s_star[i]-M_PI);
	  e1=abs(s[i] - s_star[i]+M_PI);*///*(s[i] - s_star[i]+M_PI);
      e[i] = s[i] - s_star[i];//*(s[i] - s_star[i]);
     /* if (e[i]>e0)
      {
    	  e[i]=e0;
      }
      ...
      //cout << s[i] <<" ok "<< s_star[i]<< " ok "<<e[i] << endl;
